...
-- update of new operation

-- landord dashboard ( total property + collected rent)
...
-- overdue payment ( due payment + property of tenant)
...
-- property occupancy rate ( is occupied? is rented ? under maintainance?)
...
-- Maintainance tracker with assigned staff
...
-- There we are inserting some Dummy data
...
-- 2. Force update their passwords just to be 100% safe
...
-- Adding dummy staff data

-- Assigning ahon 5 dummy done work and 4 dummy pending work 
INSERT INTO maintenance_requests (unit_id, tenant_id, title, description, priority, status, submitted_at, resolved_at) VALUES
...
-- 3. Automatically assign those 9 requests to Ahon (Assuming Ahon's staff_id is 1)
...
-- 4. Insert dummy customer reviews for Ahon
...
-- some dummy notification for ahon 
...
-- 2. Create the Staff Performance View using Aggregate Functions
