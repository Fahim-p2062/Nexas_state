DROP DATABASE IF EXISTS nexasestate;
...
CREATE TABLE notifications (
  ...
  user_role ENUM('Landlord','Tenant','Staff'),
...
CREATE TABLE audit_logs (
  ...
  record_id INT,
  performed_by INT,
  performed_by_role ENUM('Landlord','Tenant','Staff'),
...
ADD COLUMN property_id INT NULL AFTER lease_id,
ADD FOREIGN KEY (property_id) REFERENCES properties(property_id);

ALTER TABLE payments 
ADD COLUMN late_fee_amount DECIMAL(10,2) DEFAULT 0.00 AFTER amount,
ADD COLUMN rent_month VARCHAR(20) AFTER due_date,
ADD COLUMN rent_year INT AFTER rent_month;

ALTER TABLE units 
ADD COLUMN meter_number VARCHAR(50) AFTER unit_number;

ALTER TABLE audit_logs
  MODIFY COLUMN performed_by_role
...
-- =====================================================================================
-- USE CASE: We use a CHECK constraint here to guarantee that a staff review rating 
--           can never be accidentally entered as 0 or 6. It strictly forces data 
--           integrity at the database level so only ratings from 1 to 5 are accepted.
-- =====================================================================================
CREATE TABLE IF NOT EXISTS staff_reviews (
  review_id INT AUTO_INCREMENT PRIMARY KEY,
  staff_id INT NOT NULL,
  reviewer_name VARCHAR(100) NOT NULL,
  rating INT NOT NULL CHECK (rating >= 1 AND rating <= 5), -- <-- This is the CONSTRAINT
  comment TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- =====================================================================================
-- 2. REQUIREMENT: TRIGGER
-- =====================================================================================
-- USE CASE: This trigger acts as an automated listener. Whenever a new lease is 
--           inserted into the system, it automatically fires and updates the associated
--           unit's status to 'Occupied'. This prevents data inconsistency where a unit
--           might have an active lease but still be mistakenly labeled as 'Vacant'.
-- =====================================================================================
DELIMITER //
...
-- =====================================================================================
-- 3. REQUIREMENT: FUNCTION
-- =====================================================================================
-- USE CASE: This custom SQL function encapsulates business logic for calculating 
--           the total amount due for a payment. Instead of writing the math 
--           (amount + late_fee_amount) every time we query the database, we can 
--           just call this reusable function calculate_total_due(payment_id).
-- =====================================================================================
DELIMITER //
CREATE FUNCTION calculate_total_due(payment_id_param INT) 
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
  DECLARE total DECIMAL(10,2);
  SELECT (amount + late_fee_amount) INTO total 
  FROM payments 
...
-- =====================================================================================
-- 4. REQUIREMENTS: VIEW, JOIN, and AGGREGATED FUNCTIONS
-- =====================================================================================
-- USE CASE: We created this View as a virtual table to generate a performance report. 
--           - It uses JOINS to link Staff, Assignments, Requests, and Reviews together.
--           - It uses AGGREGATE FUNCTIONS (COUNT, SUM, AVG) to calculate the total 
--             tasks completed, tasks pending, and average rating for each staff member.
--           - It uses a VIEW so the frontend can query this complex report instantly 
--             without having to write the massive JOIN query every time.
-- =====================================================================================
...
SELECT 
    ...
    COUNT(DISTINCT ma.assignment_id) AS Total_Tasks_Assigned, -- AGGREGATE FUNCTION
    SUM(CASE WHEN mr.status IN ('Resolved', 'Closed') THEN 1 ELSE 0 END) AS Tasks_Completed, -- AGGREGATE FUNCTION
    SUM(CASE WHEN mr.status NOT IN ('Resolved', 'Closed') THEN 1 ELSE 0 END) AS Tasks_Pending, -- AGGREGATE FUNCTION
    COALESCE(AVG(sr.rating), 0) AS Average_Rating, -- AGGREGATE FUNCTION
    COUNT(DISTINCT sr.review_id) AS Total_Reviews -- AGGREGATE FUNCTION
FROM staff s
LEFT JOIN maintenance_assignments ma ON s.staff_id = ma.staff_id -- JOIN
LEFT JOIN maintenance_requests mr ON ma.request_id = mr.request_id -- JOIN
LEFT JOIN staff_reviews sr ON s.staff_id = sr.staff_id -- JOIN
GROUP BY s.staff_id, s.name, s.role;


-- =====================================================================================
-- DUMMY DATA INSERTION (Seeding the Database)
-- =====================================================================================
...
INSERT IGNORE INTO landlords (name, email, password_hash, contact) VALUES
('Fahim Landlord', 'fahim123@gmail.com', '$2b$10$0lFaWA4cSkJTrA8pPQiNPeEqhIm9Pkm0I.pcC.AKQi/2R687bVfqG', '01234-567890');
INSERT IGNORE INTO tenants (name, email, password_hash, nid, phone, emergency_contact) VALUES
...
INSERT IGNORE INTO staff (landlord_id, name, email, password_hash, role, phone) VALUES
(1, 'Ahon Staff', 'ahon123@gmail.com', '$2b$10$0lFaWA4cSkJTrA8pPQiNPeEqhIm9Pkm0I.pcC.AKQi/2R687bVfqG', 'Manager', '01700-112233');

UPDATE admins SET password_hash = '$2b$10$TvGQVGX39FZZUPyqhSNxA.0/rIsXozKswkOwqfOW2J9307323vpKS' WHERE email = 'bossman9@gmail.com';
UPDATE landlords SET password_hash = '$2b$10$0lFaWA4cSkJTrA8pPQiNPeEqhIm9Pkm0I.pcC.AKQi/2R687bVfqG' WHERE email = 'fahim123@gmail.com';
UPDATE tenants SET password_hash = '$2b$10$0lFaWA4cSkJTrA8pPQiNPeEqhIm9Pkm0I.pcC.AKQi/2R687bVfqG' WHERE email = 'arif123@gmail.com';
UPDATE staff SET password_hash = '$2b$10$0lFaWA4cSkJTrA8pPQiNPeEqhIm9Pkm0I.pcC.AKQi/2R687bVfqG' WHERE email = 'ahon123@gmail.com';

-- Dummy Reviews & Requests
INSERT INTO maintenance_requests (unit_id, tenant_id, title, description, priority, status, submitted_at, resolved_at) VALUES
...
SELECT request_id, 1, DATE_ADD(submitted_at, INTERVAL 1 HOUR), 'Ahon assigned'
