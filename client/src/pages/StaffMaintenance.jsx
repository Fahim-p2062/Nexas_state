...
const StaffMaintenance = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this resolved request? This cannot be undone.')) return;
    setDeletingId(id);
    try {
  ...
  const renderCard = (m, index) => (
    <div key={m.request_id} className="scroll-reveal" style={{
      ...
      opacity: m.status === 'Resolved' || m.status === 'Closed' ? 0.7 : 1,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
          ...
          {(m.status === 'Resolved' || m.status === 'Closed') && (
            <button
              onClick={() => handleDelete(m.request_id)}
              disabled={deletingId === m.request_id}
              style={{
                ...
                fontWeight: 600, cursor: deletingId === m.request_id ? 'wait' : 'pointer',
                transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '6px'
              }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(239,68,68,0.18)'}
              onMouseLeave={e => e.currentTarget.style.background = 'rgba(239,68,68,0.08)'}
            >
              🗑 {deletingId === m.request_id ? 'Deleting…' : 'Delete'}
            </button>
          )}
        </div>
      </div>
      ...
      <div style={{ fontSize: '14px', color: 'var(--text-secondary)', display: 'flex', flexWrap: 'wrap', gap: '24px', background: 'var(--bg-secondary)', padding: '16px', borderRadius: '10px' }}>
    ...
    <div style={{ animation: 'modalIn 0.5s ease', maxWidth: '800px', margin: '0 auto' }}>
      ...
      {activeRequests.length > 0 && (
        <div style={{ marginBottom: '40px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '2px', color: 'var(--text-muted)', marginBottom: '16px' }}>
            Active Tasks ({activeRequests.length})
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {activeRequests.map((m, i) => renderCard(m, i + 1))}
          </div>
        </div>
      )}
      ...
      {resolvedRequests.length > 0 && (
        <div>
          <h3 style={{ fontSize: '14px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '2px', color: 'var(--text-muted)', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
            Resolved / Closed 
            <span style={{ background: 'rgba(16,185,129,0.1)', color: '#10b981', padding: '2px 10px', borderRadius: '20px', fontSize: '12px' }}>
              {resolvedRequests.length} · Click 🗑 to clear
            </span>
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {resolvedRequests.map((m, i) => renderCard(m, i + 1))}
          </div>
        </div>
      )}
